...
export default class Board extends React.Component {
  ...
  componentDidMount() {
    ...
    this.board.addEventListener('touchmove', this.touchMoveHandler, {
      passive: false,
    });
    this.addStartTiles();
  }
  ...
  componentWillUnmount() {
    ...
    this.board.removeEventListener('touchmove', this.touchMoveHandler, {
      passive: false,
    });
  }
  ...
  isResized = (nextProps) => {
    return (
      this.props.rows !== nextProps.rows || this.props.cols !== nextProps.cols
    );
  };
  ...
  mergeTiles = () => {
    ...
    const grid = this.state.grid.map((row, r) =>
      row.map((cell, c) => {
        // There're two tiles in the given cell that need to be merged
        if (cell.get('tile').size > 1) {
          const value = cell
            .get('tile')
            .reduce((t1, t2) => t1.get('value') + t2.get('value'));

          scoreAdded += value;

          if (scoreAdded === this.props.max && !this.props.continue)
            this.props.winGame();
  ...
  insertNewTile = (grid, r, c) => {
    ...
    if (within2dList(grid, r, c)) {
      updatedGrid = grid.updateIn([r, c], (cell) =>
        cell.update('tile', (tile) =>
          tile.push(
            this.newTile({
              row: r,
              col: c,
  ...
  prepareTraversalMap = (vector) => {
    let traversal = Map({
      row: Array.apply(null, { length: this.props.rows }).map(
        Number.call,
        Number,
      ),
      col: Array.apply(null, { length: this.props.cols }).map(
        Number.call,
        Number,
      ),
    });
    ...
    if (vector.x === 1)
      traversal = traversal.update('col', (value) => value.reverse());
    if (vector.y === 1)
      traversal = traversal.update('row', (value) => value.reverse());
    ...
    traversal.get('row').forEach((r) => {
      traversal.get('col').forEach((c) => {
        ...
        if (cell.get('tile').size > 0) {
          ...
          if (
            nextPos.row !== nextPos.nextRow ||
            nextPos.col !== nextPos.nextCol
          ) {
  ...
  nextPosition = (grid, r, c, vector) => {
    ...
    // Move to the farthest possible cell in the direction of the given vector
    while (within2dList(grid, nextRow, nextCol)) {
      ...
      if (nextCell.get('tile').size === 0) {
        ...
        if (
          nextCell.get('tile').size === 1 &&
          currCell.getIn(['tile', 0, 'value']) ===
            nextCell.getIn(['tile', 0, 'value'])
        ) {
          nextPos.nextRow = nextRow;
          nextPos.nextCol = nextCol;
        }
  ...
  /**
   * Move all tiles to the given positions
